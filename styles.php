...
function moesia_custom_styles($custom) {
	...
	if ( isset($primary_color) && ( $primary_color != '#ff6b53' )) {
		$custom .= ".post-navigation .nav-previous, .post-navigation .nav-next, .paging-navigation .nav-previous, .paging-navigation .nav-next, .comment-respond input[type='submit'], button, html input[type='button'], input[type='reset'], input[type='submit'], ::selection { background-color: {$primary_color}; }"."\n";
    ...
    $custom .= ".entry-cat .cat-links { background-color: {$menu_bg}; }"."\n";
    $custom .= ".entry-cat { border-bottom: 4px solid {$menu_bg}; }"."\n";
    $custom .= "@media screen and (max-width: 991px) { .main-navigation ul ul li { background-color: {$menu_bg}; } }"."\n";
	}
	//Menu links
